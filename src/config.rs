...
pub struct Generate {
    ...
    /// Where `--sql server` writes its migrations. `--migrations-dir`
    /// beats it for a run.
    pub migrations_dir: Option<PathBuf>,
    /// How those migrations are named. Placeholders are `{version}`
    /// (`YYYYMMDDNNN`), `{date}` and `{seq}` (its halves), `{schema}`,
    /// `{table}`, `{slug}` (`<schema>_<table>_crud`) and `{tag}`;
    /// anything else is written as given. See
    /// [`crate::output::DEFAULT_MIGRATION_NAME`].
