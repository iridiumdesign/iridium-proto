        ...
        Command::Mapper {
            ...
            if opts.strategy == Strategy::Server {
                ...
                report(output::write_migration(
                    ...
                    &migration_parts(&model, migrations.tag),
...
fn mapper_target<'a>(
    ...
    mappers: bool,
    dir: Option<&'a Path>,
) -> Result<Option<(&'a Path, Option<Migrations<'a>>)>> {
    if !mappers {
        return Ok(None);
    }
    ...
    let migrations = match Strategy::from(cli.sql) {
        Strategy::Server => Some(migrations_target(cli, generate)?),
...
/// Where `--sql server` migrations go, and what `{tag}` says there.
#[derive(Clone, Copy)]
struct Migrations<'a> {
    dir: &'a Path,
    tag: Option<&'a str>,
...
/// The flags, else `migrations_dir` and `migration_tag` from the
/// config's `[generate]`.
fn migrations_target<'a>(cli: &'a Cli, generate: &'a Generate) -> Result<Migrations<'a>> {
    let dir = cli
        .migrations_dir
        .as_deref()
        .or(generate.migrations_dir.as_deref())
        .ok_or_else(|| {
            Error::Usage(
                "--sql server needs --migrations-dir, or migrations_dir under [generate]"
                    .to_string(),
            )
        })?;
    let tag = cli
        .migration_tag
        .as_deref()
        .or(generate.migration_tag.as_deref());
    Ok(Migrations { dir, tag })
}

fn migration_parts(model: &Model, tag: Option<&str>) -> output::MigrationParts {
    output::MigrationParts::new(
    ...
    match file {
        ...
        Some(config) => {
            ...
            println!(
                ...
                g.migrations_dir
                    .as_deref()
                    .map_or("(--migrations-dir)".to_string(), |d| d
                        .display()
                        .to_string()),
                g.migration_name
            );
