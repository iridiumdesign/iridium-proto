...
pub fn reconcile(existing: &str, rendered: &str) -> Option<String> {
    ...
    let mut present: BTreeMap<String, Vec<&syn::Item>> = BTreeMap::new();
    for item in &old.items {
        if let Some(key) = key(item) {
            present.entry(key).or_default().push(item);
        }
    }
    ...
    for want in &new.items {
        ...
        // A block of methods proto owns is matched against the block
        // that holds proto's methods: somebody's own `impl Item` below
        // the generated one is theirs, and the render's methods must not
        // land there a second time.
        let have = present.get(&key).and_then(|items| match want {
            syn::Item::Impl(block) if block.items.iter().any(owned) => items
                .iter()
                .find(|i| matches!(i, syn::Item::Impl(b) if b.items.iter().any(owned))),
            _ => items.last(),
        });
        match have {
...
fn key(item: &syn::Item) -> Option<String> {
    Some(match item {
        ...
        syn::Item::Impl(i) => match &i.trait_ {
            Some((_, trait_, _)) => {
                format!("impl {} for {}", normalise(trait_), impl_target(i)?)
            }
            None => format!("impl {}", impl_target(i)?),
        },
        syn::Item::Use(i) => format!("use {}", normalise(i)),
        syn::Item::Mod(i) => format!("mod {}", i.ident),
        syn::Item::Fn(i) => format!("fn {}", i.sig.ident),
        // An item a macro makes — `create_exception!(proto, Name, ..)` —
        // is what its first two arguments say it is: the rest is its
        // base and its doc, which a regeneration may reword.
        syn::Item::Macro(i) => {
            let head = i
                .mac
                .tokens
                ...
                .collect::<Vec<_>>()
                .join(",");
            format!("macro {}!({head})", normalise(&i.mac.path))
        }
        _ => return None,
    })
}
...
"#;
    ...
    /// Somebody's own `impl Item` under proto's is theirs. The render's
    /// block matches proto's, not theirs, so a method arrives once; and
    /// when only theirs is there, proto's block is added beside it.
    #[test]
    fn a_block_of_somebodys_own_is_not_where_protos_methods_go() {
        let protos = "// @generated by proto 0.1.0 — regenerate rather than rewrite.
...
impl Item {
    /// The row's key, copied into every child row it holds.
    ///
    /// proto owns this method and rewrites it when the schema changes, so
    /// edits here do not survive. Add a method of your own beside it.
...
";
        let lived_in = format!("{protos}{theirs}");
        let out = reconcile(&lived_in, protos).expect("both parse");
        assert_eq!(out, lived_in);

        // A file from before the method existed: theirs is the only
        // block, and stays as it is; proto's comes in after it.
        let before = format!(
            "// @generated by proto 0.1.0 — regenerate rather than rewrite.\n\n\
             pub struct Item {{\n    pub id: i64,\n}}\n{theirs}"
        );
        let out = reconcile(&before, protos).expect("both parse");
        assert_eq!(out.matches("impl Item {").count(), 2, "{out}");
        assert_eq!(out.matches("fn set_id_on_children").count(), 1, "{out}");
        assert!(out.contains(theirs), "{out}");
        assert!(syn::parse_file(&out).is_ok(), "{out}");
    }

    /// An exception `create_exception!` makes is an item like any
    /// other: one that is not there yet arrives, one whose doc was
    /// reworded is replaced, and one that is the same is left alone.
    #[test]
    fn an_exception_a_macro_makes_arrives_and_is_kept_up_to_date() {
        let old = "// @generated by proto 0.1.0\n\n\
            pyo3::create_exception!(proto, ProtoError, pyo3::exceptions::PyException, \"old\");\n";
        let new = "// @generated by proto 0.1.0\n\n\
            pyo3::create_exception!(proto, ProtoError, pyo3::exceptions::PyException, \"new\");\n\
            pyo3::create_exception!(proto, OperationError, pyo3::exceptions::PyException, \"ops\");\n";
        let out = reconcile(old, new).expect("both parse");
        assert_eq!(out.matches("create_exception!").count(), 2, "{out}");
        assert!(
            out.contains("ProtoError, pyo3::exceptions::PyException, \"new\""),
            "{out}"
        );
        assert!(!out.contains("\"old\""), "{out}");
        assert!(out.contains("OperationError"), "{out}");
        assert_eq!(reconcile(new, new).as_deref(), Some(new));
    }

    /// `impl Display for Error` and `impl From<E> for Error` are two
    /// blocks, not one: a method of one must never land in the other.
    #[test]
    fn a_trait_impl_is_its_own_block() {
        let old = "// @generated by proto 0.1.0\n\n\
            pub struct Error;\n\n\
        ...
        let out = reconcile(old, &new).expect("both parse");
        assert!(out.contains("write!(f, \"new\")"), "{out}");
        assert_eq!(out.matches("fn fmt").count(), 1, "{out}");
        assert_eq!(out.matches("fn from").count(), 1, "{out}");
        assert!(syn::parse_file(&out).is_ok(), "{out}");
    }
